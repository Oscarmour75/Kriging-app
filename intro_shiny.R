...
ui=fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      selectInput("kern","Kernel",choices=c("Gaussian"="gauss","Exponential"="exp","Matern 3/2"="matern3_2","Matern 5/2"="matern5_2")),
      ...
      conditionalPanel(
        ...
        sliderInput(inputId='sigopt',label='Initial standard Variance',value=0.5,min=0,max=1,step=0.001),
        actionButton("bout", "Parameters estimation"),
        radioButtons("est","Estimation method",c("Log-Likelihood"="LL","Leave-One-Out"="LOO","Log-Marginal-Posterior"="LMP")),
        ...
        sliderInput("seed", "Random Seed", min = 0, max = 100, value = 1,step=1)
...
server=function(input,output,session){
  ...
  estrec <- reactive({
    ...
    if (input$est=="Leave-One-Out"){
      "LOO"
    }
    if (input$est=="Log Maximum Posterior"){
      "LMP"
    }
  })
  ...
  k_R <- reactive({
    set.seed(input$seed)
    Kriging(y(), X(),optim=input$Opt,kernel = input$kern,parameters=parameters(),objective=estrec())
  })
...
########CHANGEMENT DES PARAMETRES SI APPUI DU BOUTON############
  ...
  observeEvent(input$bout, {
    if(input$P==FALSE) {
      updateSliderInput(session, "thetaopt", value = k_R()$theta()[1])
      updateSliderInput(session, "sigopt", value = k_R()$sigma2()[1])
    }
    if(input$P==TRUE) {
      updateSliderInput(session, "thetaopt", value = k_Rnugg()$theta()[1])
      updateSliderInput(session, "sigopt", value = k_Rnugg()$sigma2()[1])
    }
  })
